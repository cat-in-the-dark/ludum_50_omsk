...
public class Paper : MonoBehaviour
{
    ...
    public float AppendText()
    {
        if (text.Length < maxChars)
        {
            text += textTemplate;
        }
        currentProgress = Mathf.Clamp01(currentProgress + state.typingSpeed);
        var nChars = Mathf.CeilToInt(currentProgress * maxChars);
        uiText.text = text.Substring(0, nChars);
